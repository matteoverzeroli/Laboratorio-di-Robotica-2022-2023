...
Per il progetto sono stati utilizzati i seguenti metodi della classe \texttt{KDL::Path_Line}:
\texttt{ double KDL::Path_Line::PathLength()}: questo metodo ritorna la lunghezza della path con dimensione LENGTH, questo parametro non è sempre da considerarsi una lunghezza dal punto di vista fisico, inparticolare nel caso in cui ci sia una prevalente componente di rotazione
\texttt{ Frame KDL::Path_Line::Pos(double s)}: questo metodo ritorna il \texttt{Frame} sitante \texttt{s} dal frame di partenza, ciò ci permette di avere una serie di waypoints all'interno della traiettoria lineare implementando un semplice ciclo for
